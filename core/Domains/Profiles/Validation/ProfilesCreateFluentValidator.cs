...
namespace PortfolioApi.Core.Domains.Profiles.Validation
{
    ...
    public class ProfilesCreateFluentValidatorModel : AbstractValidator<Profile>
    {
        public ProfilesCreateFluentValidatorModel()
        {
            ...
            RuleFor(p => p.Info.AboutMe)
            .NotEmpty()
            .WithMessage("Please tell us about yourself. People want to know!");

            RuleFor(p => p.Info.BirthDate)
            ...
            RuleFor(p => p.Info.ImageUrl)
            .Matches(@"\.(?:jpg|gif|png)")
            .WithMessage("Image is not supported or formatted correctly")
            .When(x => !String.IsNullOrEmpty(x.Info.ImageUrl));
        }
    }
